...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildMessageReactions, // <-- ADDED FOR EMOJI
  ],
    ...
    if (tracked.channelId === message.channel.id) {
      ...
      console.log(
        `✅ ${isManager ? "Manager" : "Supervisor"} ${
          message.author.id
        } replied → cleared timers for ${msgId}`
      );
    }
  }
...
// === EVENT: SUPERVISOR or MANAGER EMOJI REACTION ===
client.on("messageReactionAdd", async (reaction, user) => {
  try {
    if (user.bot) return;

    const isSupervisor = SUPERVISORS.some((sup) => sup.id === user.id);
    const isManager = MANAGERS.includes(user.id);
    ...
    for (const [msgId, tracked] of supervisorMessages.entries()) {
      if (tracked.channelId === message.channel.id) {
        ...
        console.log(
          `✅ ${isManager ? "Manager" : "Supervisor"} ${
            user.id
          } reacted with emoji → cleared timers for ${msgId}`
        );
      }
    }
    ...
    console.error("❌ Error processing emoji reaction:", err);
  }
});
