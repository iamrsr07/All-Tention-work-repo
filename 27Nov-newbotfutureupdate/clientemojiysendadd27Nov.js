...
// === TIMER SETTINGS ===
const ONE_MINUTE = 1 * 60 * 1000;     // 1st alert
const THIRTY_SECONDS = 30 * 1000;     // 2nd alert
const TEN_SECONDS = 10 * 1000;        // manager alert
const TWO_MINUTES = 2 * 60 * 1000;    // senior alert
...
// === EVENT: USER MESSAGE — TRACK CLIENT MSG ===
client.on("messageCreate", async (message) => {
  ...
  if (message.author.id === supervisor.id || message.author.id === MANAGER_ID)
    return;
...
// === EVENT: SUPERVISOR OR MANAGER MESSAGE ===
client.on("messageCreate", async (message) => {
  if (message.author.bot) return;
...
// === EVENT: ANY USER REACTION (CLIENT OR STAFF) ===
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  // ⭐ ANY USER EMOJI cancels all timers
  for (const [msgId, tracked] of supervisorMessages.entries()) {
    if (tracked.channelId === message.channel.id) {
      ...
      console.log(`🟢 Emoji by ${user.id} → timers cleared for ${msgId}`);
    }
  }
