...
class KlaviyoDeliverabilityMonitor {
  ...
  /**
   * Send Discord notification based on score
   */
  async sendDiscordAlertForScore(accountName, metrics, score, scoreInfo) {
    if (!this.discordWebhookUrl) {
      console.log('⚠️  No Discord webhook URL configured');
      return;
    }

    const color = score < 60 ? 0xFF0000 : 0xFFA500; // Red if below 60, orange if 60-74
    const title = score < 60 ? 
      `🚨 CRITICAL: ${accountName} Deliverability Score Alert` :
      `⚠️ WARNING: ${accountName} Deliverability Score Alert`;

    let description = `**${accountName}** deliverability score has dropped below 75!\n\n`;
    description += `**${scoreInfo.emoji} Deliverability Score: ${score}/100 (Grade ${scoreInfo.grade})**\n`;
    description += `Status: ${scoreInfo.status}\n\n`;
  ...
  async monitorAll() {
    ...
    for (const account of this.accounts) {
      ...
      // Send Discord alert ONLY if score is below 75
      if (score < 75) {
        console.log(`\n🔴 ALERT: Deliverability score is below 75!`);
        await this.sendDiscordAlertForScore(account.name, metrics, score, scoreInfo);
      } else {
        console.log(`\n✅ Score is healthy (above 75)!`);
      }
    }
